...
工厂和产品一开始都是没定义的，就像产品的属性一样，都没有的。

所以产品要有一个抽象类，比如车来说，定义车类产品的对外表现形式，而没有具体的牌子。
...
subgraph 2
B-->C[Abstract Class]-->D[产品对外的表现形式]
end
subgraph 3
...
我们往常的新建一个类的方法是`new BenzCar("a");` ，那如果有很多需求难道要new很多次吗？

比如有10个人，每个人都需要一个车，它们10个人不会自己造，而是说将需求，也可以说是订单给工厂，工厂根据需求（订单）来造车。

实际上工厂模式解决的问题就是我们在需要新对象的地方，我们是将它的实例化交给专门的类来做，还是哪里用到就在哪里`new`一个出来。

**肯定是实例化交给专门的类来做啦。**

工厂内部封装实例化每个产品的方法，所以说就是**每个产品对应一个工厂**
...
书接上文，现在我们有了三个抽象工厂类
如果有个客户有某种需求，他会拿着需求依次去轮询这三个工厂，像访问链表的节点一样

最后到哪个工厂能解决需求了，哪个工厂就创建新的类

如下图所示，客户的需求在Audi得到了解决，所以Audi实例化新的对象
...
这个实际上更为抽象，一般来说只有在特定功能(比如上述)需要的时候才会用到它

一般借助抽象工厂模式来实现。

比如单例模式，使用场景很简单，就是有将一个类只能有一个对象的时候才用。

而责任链模式则可以在需要多个`if ... else`判断的时候用。

比如上文的客户的需求，如果不用链表，那么就得写三个判断来判定是否满足客户的需求。

如果有100个类呢？那么代码将会非常冗杂。
...
那么用什么样子的方式才能将不同的类串成一个链表呢？

作为一个链表中的节点，它们的指针类型应该是一样的，所以它们**统一继承自一个派生类。**

并且还要给它们增加判定是否符合客户的需求的方法。
...
新增的解决需求的功能和抽象工厂类似，需要在`ICar`中造一个新的抽象类`IHandler`出来作为解诀需求的类
...
同时`Benz, BMW, Audi`三个字类多继承自`IFactory, Ihandler`，并重写`is_valid(int x)`即验证是否满足需求x的方法
...
这里需要注意的是，在我们这个程序的设计中*Chain of Responsibility*只有一个，所以不妨用单例模式实现它
用`ChainMaster`来访问这个链表

**单例模式**
`get_instance`和`buildChain`共同实现了单例模式
在没有链表的时候才会新初始化一个
并且这个链表有且只有一个
```c++
class ChainMaster {
...
之后记得初始化`ChainMaster`中的`head`为空指针，做成全局的就行，是`ICar::IHandler *`类型的
...
当`auto p = ChainMaster::get_instance(); p; p = p->next;`时
如果`p->is_valid(req)`就要新造一个对应的实例出来了
但是这个实例需要`new BenzCar::Factory()`或者是
调用`ICar::create()`来返回新的实例
但是我们这个是`IHandler`类，`IHandler`是访问不到`IFactory`类内属性的
但是`BenzCar`是公有多继承自`ICar::IFactory`和`ICar::IHandler`

我们可以利用这个性质来访问`IFactory::create()`

将`IHandler::do_it()`设成纯虚函数
在每个具体类中重写`do_it()`，返回`this->create();`
```c++
        virtual ICar *create() {
            return new BenzCar();
        }
